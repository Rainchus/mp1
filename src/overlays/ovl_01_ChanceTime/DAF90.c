#include "ChanceTime.h"

void func_800FC390_DAF90(void) {
    D_80101760 = func_800174F4(0x130001, 0x289);
    func_800258EC(D_80101760, 4, 4);
}

INCLUDE_ASM(const s32, "overlays/ovl_01_ChanceTime/DAF90", func_800FC3D0_DAFD0);

INCLUDE_ASM(const s32, "overlays/ovl_01_ChanceTime/DAF90", func_800FCC18_DB818);

INCLUDE_ASM(const s32, "overlays/ovl_01_ChanceTime/DAF90", func_800FD0AC_DBCAC);

INCLUDE_ASM(const s32, "overlays/ovl_01_ChanceTime/DAF90", func_800FD7DC_DC3DC);

INCLUDE_ASM(const s32, "overlays/ovl_01_ChanceTime/DAF90", func_800FDDD4_DC9D4);

INCLUDE_ASM(const s32, "overlays/ovl_01_ChanceTime/DAF90", func_800FE554_DD154);

void func_800FE97C_DD57C(s32 arg0) {
    if (((func_800FEC4C_DD84C(arg0, 80.0f, D_800F3FB0)) != 0) && (D_8010131A == 0)) {
        D_8010131A = 1;
    }

    D_801012B4 = 1;
    D_801012E1 = 0;
}

void func_800FE9D8_DD5D8(omObjData* arg0) {
    unkGlobalStruct_02* temp_s1;

    arg0->model[0] = func_80023FC8(D_80101760);
    func_800258EC(arg0->model[0], 4, 4);
    arg0->model[1] = func_800174F4(0x130002, 0x699);
    arg0->trans.x = 140.0f;
    arg0->trans.y = 230.0f;
    arg0->trans.z = 1400.0f;
    arg0->scale.x = arg0->scale.y = arg0->scale.z = 1.5f;
    arg0->rot.x = arg0->rot.y = arg0->rot.z = 0.0f;
    func_80025798(arg0->model[1], arg0->trans.x, arg0->trans.y, arg0->trans.z);
    func_80025830(arg0->model[1], 0.8f, 0.8f, 0.8f);
    func_800257E4(arg0->model[1], 0.0f, 0.0f, 0.0f);
    temp_s1 = HuMemTempDirectMalloc(sizeof(unkGlobalStruct_02), 0x7918);
    arg0->unk_50 = temp_s1;
    func_8009B770(temp_s1, 0, sizeof(unkGlobalStruct_02));
    temp_s1->unk_04 = 1;
    temp_s1->unk_05 = 3;
    temp_s1->unk_28 = HuMemTempDirectMalloc(sizeof(unkGlobalStruct_01), 0x7918);
    func_8009B770(temp_s1->unk_28, 0, sizeof(unkGlobalStruct_01));
    arg0->func_ptr = &func_800FE97C_DD57C;
}
